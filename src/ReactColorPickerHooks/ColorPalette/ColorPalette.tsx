...
export function ColorPalette({ hue, setColor }: IColorPaletteProps) {
    ...
    useEffect(() => {
        ...
        const {red, green, blue} = hsl2rgb({ hue, saturation: 100, lightness: 50 });
        ...
        // markerRef.current.style.right = '0';
    ...
    const onClick = useCallback((event: React.MouseEvent<HTMLCanvasElement>) => {
        ...
        markerRef.current.style.top = `${y - 10}px`;
        markerRef.current.style.left = `${x - 10}px`;
