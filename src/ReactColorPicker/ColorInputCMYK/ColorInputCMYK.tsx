...
function cmyk2rgb({ cyan, magenta, yellow, black }: Record<string, number>) {
    ...
    return {
        red: 255 * (1 - cyan / 100) * key,
        green: 255 * (1 - magenta / 100) * key,
        blue: 255 * (1 - yellow / 100) * key,
    }
}
...
export function ColorInputCMYK({ color, setColor }: IColorInputCMYKProps) {
    ...
    return (
        <div className={styles.container}>
            <input name='cyan' className={styles.input} type="number" min={0} max={100} maxLength={3} value={cyan} onChange={onChange} />
            <label>C</label>
            <input name='magenta' className={styles.input} type="number" min={0} max={100} maxLength={3} value={magenta} onChange={onChange} />
            <label>M</label>
            <input name='yellow' className={styles.input} type="number" min={0} max={100} maxLength={3} value={yellow} onChange={onChange} />
            <label>Y</label>
            <input name='black' className={styles.input} type="number" min={0} max={100} maxLength={3} value={black} onChange={onChange} />
            <label>K</label>
        </div>
    );
}